...
> **⚠️ 先别急着拆——先在电脑上试一次**
>
> 很多玩具会出现「**手机版 Intiface 连不上、电脑版却一连就好**」。这多半不是玩具
> 不被支持，而是**手机版 Intiface 内置的设备清单更新慢**，落后于电脑版。所以手机上
> 连不上，先别下结论：
>
> 1. 找台带蓝牙的电脑，装**最新版 Intiface Central 桌面版**，扫描试试。能连上、还
>    显示出你玩具的名字，就说明官方已经支持——按 [USAGE.md](USAGE.md) 的「电脑组合」
>    路线用就行，本文的 DIY 完全不用碰。
> 2. 想先确认的话：在 nRF Connect 或 Intiface 里看玩具的**广播名**——一串四个字符的
>    代号（例如 `QCSW`），拿去 [iostindex.com](https://iostindex.com/) 搜，搜得到就是支持。
>
> **只有电脑版也连不上、清单里也查无此物，才需要往下 DIY。**
...
3. 酒馆必须走 **HTTP** 访问——HTTPS 云酒馆会以混合内容为由拦截发往局域网
   ...
   用 `http://电脑IP:酒馆端口` 打开。注意酒馆默认拒绝局域网访问，
   要改 `config.yaml` 放行（`listen: true` + 白名单），
   步骤见 [USAGE.md](USAGE.md) 7.5「iPhone + 电脑组合：分步」第 1 条
...
**第一步：先排除「其实被支持、只是手机版没跟上」。** 在电脑上用最新版 Intiface
Central 桌面版连一次（原因见本文开头）。电脑版能连，直接用，别 DIY。

**第二步：确认真没人支持。** 把玩具广播名（如 `QCSW`）拿去
[iostindex.com](https://iostindex.com/) 搜，查无此物，才轮到 DIY。

**第三步：判断 DIY 能不能成。** 用 nRF Connect 能扫到并连上、且玩具有自己的私有
服务（Service UUID 不是标准的 `0x1800`/`0x1801`），就大概率能做。

**但有一种玩具做不了——反编译时看到下面任一迹象，趁早收手别白费功夫：**

- 官方 App 是 **Flutter** 做的：反编译出来只有一堆单字母乱码类名，真正的指令和
  UUID 都藏在 `libapp.so` 里，翻不出来。
- 代码里有**加密握手**痕迹（搜到 `aes_ccm`、`ECDH`、`_secureHandshake`、
  `pointycastle`、`en_des_command` 这类词）：这种玩具每次连接都临时商定一把"暗号
  钥匙"，指令加密还防重放，抓到的字节一两秒就失效——等于要把官方 App 整套加密
  通道重写一遍，远超 DIY 范畴。
- 连上后玩具几十秒内自己断开（nRF 日志出现 `terminated by peer (status 19)`）：
  它要求先对暗号才肯保持连接，不会对就被踢。

> **看到加密也先别慌**：有的玩具**同时**有加密通道和普通控制通道——加密那条往往
> 是"社交密语／固件升级"之类的附加功能，真正控制震动的是另一条普通通道。所以只要
> Intiface 电脑版认得它，就交给 Intiface，你不用管它加不加密（曾有玩具就是如此：
> App 里那套加密只是密语功能，Intiface 走的是另一条普通通道，照样能控）。
