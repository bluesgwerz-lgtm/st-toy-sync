...
顶栏也会多一个心跳图标（位于角色管理图标左侧倒数第二位），那是状态灯兼急停键（见下）。
...
- **⏹ 全部停止**：常驻急停按钮
- **顶栏心跳图标**：状态灯兼急停键，不用打开设置面板
  - 颜色：灰=已禁用 · 橙=已启用但没连上 Intiface · 正常色=待机 · 粉色跳动=正在播放
  - **单击=立即全停**（禁用状态下也有效）
  - 双击或长按约 0.6 秒=启用⇄禁用总开关

## 安全设计

- 顶栏常驻心跳急停图标，单击立即全停，禁用状态下同样有效
- 关闭总开关、切换后端时自动发送停止
